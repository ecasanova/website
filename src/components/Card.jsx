        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-md my-4">
            ...
            {childs && (
                <div className="mt-4 space-y-2 text-xs">
                    <p className="text-gray-500 dark:text-gray-400 text-xs">Promotions or title changes in the same company:</p>
                    <ul className='ml-4'>
                        {childs.map((child, index) => (
                            <li className={`relative pl-6 before:content-[''] before:absolute before:left-0 before:top-1 before:bg-black dark:before:bg-white before:w-2.5 before:h-2.5 before:rounded-full ${index !== childs.length - 1 ? 'after:content-[""] after:absolute after:left-[4px] after:top-2.5 after:w-0.5 after:h-full after:bg-black dark:after:bg-white' : ''}`} key={index}>
                                <Child {...child}  />
                            </li>
                        ))}
                    </ul>
